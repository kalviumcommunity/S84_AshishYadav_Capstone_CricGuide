...
const TrainerTrainee = () => {
 ...
 const handleLink = async () => {
  try {
    const res = await axios.post('http://localhost:5000/api/trainertrainee/link', { code, role });
    alert(res.data.message);

    // 🔁 If trainer, fetch trainee's video
    if (role === 'trainer') {
      const videoRes = await axios.get(`http://localhost:5000/api/trainertrainee/video/${code}`);
  ...
  const handleUpload = async (e) => {
    ...
    try {
      await axios.post('http://localhost:5000/api/trainertrainee/upload', formData);
  ...
  const handleReport = async () => {
    try {
      await axios.post('http://localhost:5000/api/trainertrainee/report', { code, report, grade });
  ...
  useEffect(() => {
    const fetchVideo = async () => {
      if (role === 'trainer' && code) {
        try {
          const res = await axios.get(`http://localhost:5000/api/trainertrainee/video/${code}`);
